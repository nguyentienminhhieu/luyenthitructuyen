<template>
  <div
    class="ml-4 mb-2 border-2 border-gray-200 rounded-xl max-h-[200px] overflow-y-auto"
  >
    <!-- <h1 class="pl-2">Câu trả lời: {{ this.answer.id }}</h1> -->
    <div class="m-1 p-2 flex justify-between">
      <div class="flex flex-col">
        <textarea
          v-if="inputTitle"
          v-model="AnswerContent"
          placeholder="Nhập tiêu đề..."
          class="w-[500px] rounded p-2 m-2 outline-none resize-none"
        ></textarea>
        <label v-if="answerTitle" class="w-[500px] break-words">{{
          AnswerContent
        }}</label>
        <div class="my-5">
          <label
      ...
      <button v-if="inputTitle" @click="saveAnswer">
        <i
          class="fa-regular fa-circle-check text-gray-500 hover:text-gray-700"
        ></i>
      </button>
      ...
      <div class="text-center">
        <button
          class="rounded-full p-2 bg-red-500 hover:bg-red-600"
          @click="deleteAnswer"
        >
